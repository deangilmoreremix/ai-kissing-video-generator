            ...
            <div className="flex items-center">
              ...
              <div className="relative">
                <UserButton
                  ...
                  appearance={{
                    elements: {
                      avatarBox: "h-9 w-9 rounded-r border-y border-r border-divider",
                    },
                  }}
                />
              </div>
      ...
      {isOpen && (
        <div className="absolute top-full left-0 right-0 z-[200] glass-dropdown border-b border-divider shadow-2xl py-4 px-6 md:hidden animate-fade-in">
          <nav className="flex flex-col gap-3">
            ...
            {isSignedIn ? (
              <div className="flex w-full items-center justify-center gap-2 rounded bg-bg-card py-3 text-sm font-bold border border-divider mt-2">
                <UserButton afterSignOutUrl="/" />
                <span className="text-primary-text">{user?.fullName || user?.primaryEmailAddress?.emailAddress}</span>
              </div>
            ) : (
              <Link
                href="/login"
